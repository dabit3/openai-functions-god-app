...
import setModel from './util'

type OpenAIConfig = {
	model: string
}

type SetModelErrorObject = {
	error?: any
	name?: string
}
...
const data: OpenAIConfig = { model: '' }

export async function POST(req: NextRequest, res: NextResponse) {
	try {
		//
		// Check object to see if we set it in a prior request
		//
		if (!data.model) {
			try {
				//
				// Change this to `gpt-4` if you have it enabled in your OpenAI account
				//
				console.log('Setting model...')
				const setModelReturn: SetModelErrorObject = await setModel(
					'gpt-3.5-turbo-16k-0613'
				)
				const { error, name } = setModelReturn
				if (error) {
					return NextResponse.json({
						error: error.message,
					})
				}
				data.model = name as string
			} catch (err: any) {
				return NextResponse.json({ error: err.message as string })
			}
		}
